...
static void msm_hsusb_vbus_power(struct msm_otg *motg, bool on)
{
	...
	if (!vbus_otg && !gpio_is_valid(vbus_gpio)) {
...
static int msm_otg_switch_mode(struct msm_otg *motg, char const *mode,
		size_t len)
{
	struct usb_phy *phy = &motg->phy;
	struct usb_otg *otg = motg->phy.otg;
	...
	if (len == 4 && !strncmp(mode, "host", 4)) {
		req_mode = USB_HOST;
	} else if (len == 10 && !strncmp(mode, "peripheral", 10)) {
		req_mode = USB_PERIPHERAL;
	} else if (len == 4 && !strncmp(mode, "none", 4)) {
		req_mode = USB_NONE;
	} else {
		dev_err(phy->dev, "Invalid mode %.*s\n", len, mode);
		status = -EINVAL;
		goto out;
	}
	...
	case USB_NONE:
		...
		case OTG_STATE_B_SUSPEND:
			...
			break;
		default:
			dev_err(phy->dev, "Invalid mode request");
			status = -EINVAL;
			goto out;
		}
		break;
	case USB_PERIPHERAL:
		...
		case OTG_STATE_A_HOST:
			set_bit(ID, &motg->inputs);
			set_bit(B_SESS_VLD, &motg->inputs);
			if (motg->quick_host_device_switch) {
				dev_dbg(phy->dev, "Quick host->device switch\n");
				work = 0;
	...
	case USB_HOST:
		...
		case OTG_STATE_B_PERIPHERAL:
		case OTG_STATE_B_SUSPEND:
			dev_err(phy->dev, "device->host switch\n");
			clear_bit(ID, &motg->inputs);
...
enum usb_mode_type msm_otg_get_mode(struct msm_otg const *motg)
{
	struct usb_otg *otg = motg->phy.otg;
	enum usb_mode_type ret;
				...
				size_t count, loff_t *ppos)
{
	...
	ret = msm_otg_switch_mode(motg, buf, len);
	if (ret) {
		status = ret;
		goto out;
	}
...
static ssize_t otg_role_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
...
static ssize_t otg_role_store(struct device *dev,
		struct device_attribute *attr, char const *buf, size_t len)
{
	struct msm_otg *motg = dev_get_drvdata(dev);
	int ret;
...
static DEVICE_ATTR(otg_role, S_IRUGO | S_IWUSR, otg_role_show, otg_role_store);
...
struct msm_otg_platform_data *msm_otg_dt_to_pdata(struct platform_device *pdev)
{
	...
	vbus_gpio = of_get_named_gpio(node, "qcom,hsusb-otg-vbus-gpio", 0);
	if(vbus_gpio > 0) {

		if(!gpio_is_valid(vbus_gpio)) {
			pr_err("usb: Unable to get vbus_gpio\n");
			return NULL;
		}

		if (gpio_request(vbus_gpio, "msm_hsusb_vbus")) {
			pr_err("usb: Unable to register vbus_gpio\n");
			return NULL;
		}

		gpio_direction_output(vbus_gpio, 1);
	} else {
		pr_warn("usb: No vbus_gpio parameter defined\n");
	}
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(pdev->dev.of_node,
		...
		motg->quick_host_device_switch = true;
	}
	...
	if (motg->pdata->otg_control == OTG_USER_CONTROL)
		device_create_file(&pdev->dev, &dev_attr_otg_role);
...
static int msm_otg_remove(struct platform_device *pdev)
{
	...
	if (motg->pdata->otg_control == OTG_USER_CONTROL)
		device_remove_file(&pdev->dev, &dev_attr_otg_role);
